...
class BlenderNCNodeSelection(bpy.types.Menu):
    ...
    def draw(self, context):
        self.layout.operator(
                    "node.add_node", text="Select Variable"
        ).type = DatacubeVariable.bl_idname
        self.layout.operator(
                    "node.add_node", text="Select Index"
        ).type = DatacubeSelect.bl_idname
        self.layout.operator(
                    "node.add_node", text="Slice Coordinate"
        ).type = DatacubeSlice.bl_idname
