...
from .utils import create_datastruct, get_datacube_path
...
class Import_OT_CreateGrid(bpy.types.Operator):
    ...
    def execute(self, context):
        if self.node_name == "" or self.node_tree == "":
            self.report(
                {"WARNING"},
                translate("blendernc", "BlenderNC: Node name or node tree is not set."),
            )
            return {"CANCELLED"}
        ...
        for input in grid_node.inputs:
            if input.is_linked:
                ...
                grid_coords[input.name] = {
                    "name": linked_socket.name,
                    "size": datastruct.dict[datastruct.filename][
                        linked_socket.name
                    ].shape,
                }
        ...
        grid_nodetree.links.new(
            MeshGrid_node.outputs["Mesh"], output_node.inputs["Geometry"]
        )
        ...
        if len(x_size) == 1 and len(y_size) == 1:
            ...
            self.report(
                {"WARNING"},
                translate("blendernc", "BlenderNC: No valid coordinates found"),
            )
            return {"CANCELLED"}
        ...
        if x_size != 1 and y_size != 1:
            x = grid_node.datastruct[grid_node.filename][
                grid_coords.get("X").get("name", 1)
            ].values
            y = grid_node.datastruct[grid_node.filename][
                grid_coords.get("Y").get("name", 1)
            ].values
            ...
            z = grid_node.datastruct[grid_node.filename][
                grid_coords.get("Z").get("name", 1)
            ].values
        ...
        if len(x_size) > 2 or len(y_size) > 2 or len(z_size) > 2:
            self.report(
                {"ERROR"},
                translate(
                    "More than 2 dimensions were found, only 2D grids are supported by the Grid node. For 3D grids, please use the volume node."
                ),
            )
            return {"CANCELLED"}
