...
class NodesDecorators(object):
    ...
    def amIconnected(node):
        ...
        if outputs.keys():
            if outputs.is_linked and outputs.links:
                ...
                connections['output']=[link.from_node.bl_idname for link in output_links]
        return connections
    ...
    def get_data_from_node(node):
        ...
        if node_parent.blendernc_dataset_identifier in node_parent.blendernc_dict.keys():
            dataset = node_parent.blendernc_dict[node.blendernc_dataset_identifier].copy()
            node.blendernc_dict[node.blendernc_dataset_identifier] = dataset
