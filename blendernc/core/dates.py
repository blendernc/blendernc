...
def get_item_days(self, context):
    ...
    for datetime in datetimes:
        if dt2cal(datetime)[1] == selected_month and dt2cal(datetime)[0] == selected_year:
            dataset_days_in_month.append(dt2cal(datetime)[2])
        else:
            break

    return [(str(day),str(day),str(day),"",int(day)) for day in dataset_days_in_month]
...
def get_item_month(self, context):
    ...
    return [(str(month),str(month),str(month),"",int(month)) for month in dataset_months_in_years]
